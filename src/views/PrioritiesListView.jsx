          ...
          {promotions.map((p) => (
            <div key={p.promo_id} className="bg-white rounded-2xl shadow-sm border border-green-4/8 p-4 flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                {role === 'hq' && p.submission_status === 'draft' && onEditPromo && (
                ...
                {role === 'hq' && p.submission_status === 'draft' && onSubmitPromo && (
                  <button onClick={() => onSubmitPromo(p.promo_id)} className="px-3 py-1.5 rounded-lg bg-green-3 hover:bg-green-4 text-white text-xs font-bold transition-colors">Submit</button>
                )}
                {/* Reporting can only be requested once a priority is submitted */}
                {p.submission_status && p.submission_status !== 'draft' && onAddRequest && (
                  <RequestButtons
                    size="xs"
                    types={['reporting']}
