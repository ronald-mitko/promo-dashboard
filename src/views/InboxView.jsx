...
import { REQUEST_TYPE_LABELS, REQUEST_TYPES, WORKFLAG_REASONS, WORKFLAG_FREQUENCIES } from '../lib/constants'
...
export default function InboxView({ session, promotions, requests, onApprovePromo, onRejectPromo, onApproveRequest, onRejectRequest, onExportRequest, onExportPromo }) {
  ...
  const myRequests = requests.filter((r) => (r.routed_rcsm === myId || !r.routed_rcsm) && r.status !== 'draft')
  const items = [
                  ...
                  <div key={it.key} className="p-4">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <ApprovalControls
                        ...
                        approveFrequencies={!isPromo && it.raw.type === REQUEST_TYPES.WORKFLAG ? WORKFLAG_FREQUENCIES : null}
                        onApprove={(reason, frequency) => (isPromo ? onApprovePromo(it.key) : onApproveRequest(it.key, reason, frequency))}
