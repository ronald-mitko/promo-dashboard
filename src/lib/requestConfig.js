...
export const REQUEST_CONFIG = {
  ...
  [REQUEST_TYPES.WORKFLAG]: {
    ...
    steps: [
      ...
      { id: 'stores', label: 'Stores', Component: StoresStep, validate: (s) => s.stores.length > 0 },
      { id: 'items', label: 'Items', Component: ItemsStep, validate: (s) => s.items.length > 0 },
      { id: 'details', label: 'Details', Component: WorkflagDetailsStep, validate: (s) => !!s.reasonCode && !!s.startDate && !!s.endDate },
    ...
    summarize: (s) => [
      ...
      { label: 'Reason', value: s.reasonCode + (s.frequency === 'every_call' ? s.reasonCode : '') },
    ...
    buildRecord: (s, refData) => ({
      ...
      comment: s.comment,
      payload: { reasonCode: s.reasonCode, frequency: s.frequency, startDate: s.startDate, endDate: s.endDate },
    }),
  },
